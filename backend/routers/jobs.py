...
def get_job_analytics(
    ...
    """
    Returns high-level insights for a specific job posting.
    Aggregates candidate match logic.
    """
    # Simply reuse the candidate matching logic for data aggregation 
    # (In a production highly-scaled app, this would be computed async or cached)
    candidates = get_ranked_candidates_for_job(job_id, current_user, db, matching_service)
    ...
    avg_score = sum(c.match_score for c in candidates) / total_candidates
    avg_exp = sum(c.experience_years for c in candidates) / total_candidates
    
    # Aggregate missing skills to find the most common gaps
    ...
    return {
        ...
        "average_match_score": float(round(avg_score * 100, 2)),
        "average_experience_years": float(round(avg_exp, 1)),
        "top_missing_skills": [{"skill": k, "count": v} for k, v in sorted_missing[:5]]
    }
